...
4. **Public gateway HTTP** — a running Nest application forwards every one of
   the 58 explicit POST routes to a fake private HTTP worker, validates options,
   preserves safe results, maps failures, publishes OpenAPI, and keeps v1
   adapters operational.
5. **UI** — all 57 manifest entries have valid defaults, controls, icons,
...
6. **Full-stack smoke** — production builds of the worker, API, and web app run
   ...
   private worker through all 58 routes. All 57 canonical UI URLs receive
...
| Decode / encode pixels                  | parsed raw ZIP layout and exact RGB/RGBA pixel round trips                             |
| Base64 / data URLs                      | source byte preservation, conversion, canonical padding and MIME validation            |
| Validate image                          | full decode rejects truncated pixels, animation, and invalid input; accepts CMYK+alpha |
| Color space / channels                  | RGB, grayscale, CMYK JPEG/TIFF, exact channel values and alpha                         |
| Duotone / posterize / solarize / levels | known pixel transforms, amount endpoints, quantization and gamma                       |
| Color matrix / convolution / morphology | custom coefficients, presets, channel isolation and alpha preservation                 |
| Replace color / chroma key / noise      | distance tolerance, alpha transitions, seeded reproducibility                          |
| Affine / auto-orient                    | transform geometry, invertibility, oriented pixels and cleared EXIF                    |
| Vignette / shadow / reflection          | radial falloff, alpha-derived shadows and reflected fading pixels                      |
| Tile / slice / sprite sheet             | placement, odd-sized lossless reconstruction and manifest coordinates                  |
| Icon set                                | requested PNG sizes, ICO directory offsets and embedded 32-bit RGBA PNG entries        |
| Pixel inspector / fingerprint           | exact RGBA, both 64-bit hashes, source SHA-256 and wide input analysis                 |
| Region masking                          | solid/blur/pixelate patches, unchanged outside pixels and region bounds                |
...
contains 58 POST endpoints for 57 tools.

## Cross-cutting fixtures and failures

The test matrix also covers:

- production dependency/import boundaries keeping native work out of the gateway/UI/contracts;
- effect and layout raw-buffer caps before allocation, transparency, CMYK and EXIF orientation;
- capacity rejection before upload buffering and slot retention after response timeouts;
- HEIC container disposal on success, error, oversize, and multi-image rejection;
